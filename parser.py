...
class Parser():
    ...
    UNIT = 'unit'
    COUNT = 'count'
    SIZE_UNIT = 'size_unit'     # Unit for disk quota
    QUOTA = 'quota'   # Value of disk quota for project   
    RAW_REGEX = 'raw_regex'
    RUNHOUR = 'runhour'
    RUNMINUTE = 'runminute'
    TIMEZONE = 'timezone'
    ...
    def create_internal_representation(self):
        for project in self.config_yaml:
            ...
            if project == '.regex':
                raise Exception('Reg expressions is not allowed in that config')
            #     self.parse_regex(self.config_yaml['.regex'])
            #     continue
            unit, count, size, quota = self.unit_count(self.config_yaml.get(project, {}).get(self.DELETE, {}))
            
            delete[self.UNIT] = unit
            delete[self.COUNT] = count
            delete[self.SIZE_UNIT] = size
            delete[self.QUOTA] = quota
            if project == '.defaults':
    ...
    #         quota, size = self.qouta_size(regex.get(self.DELETE, {}))
    ...
    def unit_count(self, delete_node):
        ...
        unit = self.default_time_unit
        count = self.default_count
        size = self.default_size_unit
        quota = self.default_size_quota
        for key in delete_node.keys():
            if key in self.allowed_units:
                unit = key if key != 'weeks' else 'days'
                count = int(delete_node[key]) if key != 'weeks' else int(delete_node[key]) * 7
            elif key == 'size':
                 quota = int(delete_node[key])    
            else:
                raise ValueError('Invalid configuration. [{0}] is not allowed unit.'.format(key))
        return (unit, count, size, quota)
